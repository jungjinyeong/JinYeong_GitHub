...
public class OrangutanBT : MonoBehaviour, ISjBehaviorTree
{
    // 리프노드 제외한 노드 생성
    #region Node 생성
    ...
    // 리프노드 생성
    #region Task 클래스 생성
    private SearchNearestTarget<OrangutanNpc> searchNearestTarget;
    private MoveToTarget<OrangutanNpc> moveToTarget;
    private AttackTargetDefault<OrangutanNpc> attackTargetDefault;
    private CheckTargetInRange<OrangutanNpc> checkTargetInRange;
    private KillNpc<OrangutanNpc> killNpc;
    private RotateToTarget<OrangutanNpc> rotateToTarget;
    #endregion

    #region AI 두뇌
    private IEnumerator behaviorProcess;

    private Brain<OrangutanNpc> brain;
    #endregion

    public void InitBT()
    {
        // Brain 추가.
        brain = GetComponent<OrangutanNpc>().NpcBrain;

        // NPC 가 할 Task 할당
        searchNearestTarget = new SearchNearestTarget<OrangutanNpc>(brain, 1 << LayerMask.NameToLayer("Player"), 30f);
        moveToTarget = new MoveToTarget<OrangutanNpc>(brain);
        checkTargetInRange = new CheckTargetInRange<OrangutanNpc>(brain, brain.NpcController.NpcAttackRange);
        attackTargetDefault = new AttackTargetDefault<OrangutanNpc>(brain);
        killNpc = new KillNpc<OrangutanNpc>(brain);
        rotateToTarget = new RotateToTarget<OrangutanNpc>(brain);
