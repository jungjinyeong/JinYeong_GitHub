...
public class MickeyBT : MonoBehaviour, ISjBehaviorTree
{
    ...
    // 리프노드 생성
    #region Task 클래스 생성
    private SearchNearestTarget<MickeyNpc> searchNearestTarget;    
    private MoveToTarget<MickeyNpc> moveToTarget;
    private AttackTargetDefault<MickeyNpc> attackTargetDefault;
    private CheckTargetInRange<MickeyNpc> checkTargetInRange;
    private KillNpc<MickeyNpc> killNpc;
    private RotateToTarget<MickeyNpc> rotateToTarget;
    private PlayerMovement<MickeyNpc> playerMovement;
    #endregion

    #region AI 두뇌
    private IEnumerator behaviorProcess;

    private Brain<MickeyNpc> brain;
    #endregion

    public void InitBT()
    {
        // Brain 추가.
        brain = GetComponent<MickeyNpc>().NpcBrain;

        // NPC 가 할 Task 할당
        searchNearestTarget = new SearchNearestTarget<MickeyNpc>(brain, gameObject.tag == "Enemy" ? 1 << LayerMask.NameToLayer("Player") : 1 << LayerMask.NameToLayer("Enemy"));
        moveToTarget = new MoveToTarget<MickeyNpc>(brain);
        checkTargetInRange = new CheckTargetInRange<MickeyNpc>(brain, brain.NpcController.NpcAttackRange);
        attackTargetDefault = new AttackTargetDefault<MickeyNpc>(brain);
        killNpc = new KillNpc<MickeyNpc>(brain);
        rotateToTarget = new RotateToTarget<MickeyNpc>(brain);
        playerMovement = new PlayerMovement<MickeyNpc>(brain);
