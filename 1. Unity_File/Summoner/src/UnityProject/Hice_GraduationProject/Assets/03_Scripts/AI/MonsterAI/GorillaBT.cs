...
public class GorillaBT : MonoBehaviour, ISjBehaviorTree
{
    // 리프노드 제외한 노드 생성
    #region Node 생성
    ...
    // 리프노드 생성
    #region Task 클래스 생성
    private SearchNearestTarget<GorillaNpc> searchNearestTarget;
    private MoveToTarget<GorillaNpc> moveToTarget;
    private AttackTargetDefault<GorillaNpc> attackTargetDefault;
    private CheckTargetInRange<GorillaNpc> checkTargetInRange;
    private KillNpc<GorillaNpc> killNpc;
    private RotateToTarget<GorillaNpc> rotateToTarget;
    #endregion

    #region AI 두뇌
    private IEnumerator behaviorProcess;

    private Brain<GorillaNpc> brain;
    #endregion

    public void InitBT()
    {
        // Brain 추가.
        brain = GetComponent<GorillaNpc>().NpcBrain;

        // NPC 가 할 Task 할당
        searchNearestTarget = new SearchNearestTarget<GorillaNpc>(brain, 1 << LayerMask.NameToLayer("Player"), 30f);
        moveToTarget = new MoveToTarget<GorillaNpc>(brain);
        checkTargetInRange = new CheckTargetInRange<GorillaNpc>(brain, brain.NpcController.NpcAttackRange);
        attackTargetDefault = new AttackTargetDefault<GorillaNpc>(brain);
        killNpc = new KillNpc<GorillaNpc>(brain);
        rotateToTarget = new RotateToTarget<GorillaNpc>(brain);
