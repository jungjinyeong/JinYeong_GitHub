...
public class ChimpanzeeBT : MonoBehaviour, ISjBehaviorTree
{
    // 리프노드 제외한 노드 생성
    #region Node 생성
    ...
    // 리프노드 생성
    #region Task 클래스 생성
    private SearchNearestTarget<ChimpanzeeNpc> searchNearestTarget;
    private MoveToTarget<ChimpanzeeNpc> moveToTarget;
    private AttackTargetDefault<ChimpanzeeNpc> attackTargetDefault;
    private CheckTargetInRange<ChimpanzeeNpc> checkTargetInRange;
    private KillNpc<ChimpanzeeNpc> killNpc;
    private RotateToTarget<ChimpanzeeNpc> rotateToTarget;
    #endregion

    #region AI 두뇌
    private IEnumerator behaviorProcess;

    private Brain<ChimpanzeeNpc> brain;
    #endregion

    public void InitBT()
    {
        // Brain 추가.
        brain = GetComponent<ChimpanzeeNpc>().NpcBrain;

        // NPC 가 할 Task 할당
        searchNearestTarget = new SearchNearestTarget<ChimpanzeeNpc>(brain, 1 << LayerMask.NameToLayer("Player"), 30f);
        moveToTarget = new MoveToTarget<ChimpanzeeNpc>(brain);
        checkTargetInRange = new CheckTargetInRange<ChimpanzeeNpc>(brain, brain.NpcController.NpcAttackRange);
        attackTargetDefault = new AttackTargetDefault<ChimpanzeeNpc>(brain);
        killNpc = new KillNpc<ChimpanzeeNpc>(brain);
        rotateToTarget = new RotateToTarget<ChimpanzeeNpc>(brain);
