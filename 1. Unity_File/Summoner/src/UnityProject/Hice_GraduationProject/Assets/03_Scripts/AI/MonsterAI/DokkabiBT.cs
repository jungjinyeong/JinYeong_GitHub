...
public class DokkabiBT : MonoBehaviour, ISjBehaviorTree
{
    ...
    // 리프노드 생성
    #region Task 클래스 생성
    private SearchNearestTarget<DokkabiNpc> searchNearestTarget;    
    private MoveToTarget<DokkabiNpc> moveToTarget;
    private AttackTargetDefault<DokkabiNpc> attackTargetDefault;
    private CheckTargetInRange<DokkabiNpc> checkTargetInRange;
    private KillNpc<DokkabiNpc> killNpc;
    private RotateToTarget<DokkabiNpc> rotateToTarget;
    #endregion

    #region AI 두뇌
    private IEnumerator behaviorProcess;

    private Brain<DokkabiNpc> brain;
    #endregion

    public void InitBT()
    {
        // Brain 추가.
        brain = GetComponent<DokkabiNpc>().NpcBrain;

        // NPC 가 할 Task 할당
        searchNearestTarget = new SearchNearestTarget<DokkabiNpc>(brain, 1 << LayerMask.NameToLayer("Player"), 30f);
        moveToTarget = new MoveToTarget<DokkabiNpc>(brain);
        checkTargetInRange = new CheckTargetInRange<DokkabiNpc>(brain, brain.NpcController.NpcAttackRange);
        attackTargetDefault = new AttackTargetDefault<DokkabiNpc>(brain);
        killNpc = new KillNpc<DokkabiNpc>(brain);
        rotateToTarget = new RotateToTarget<DokkabiNpc>(brain);
