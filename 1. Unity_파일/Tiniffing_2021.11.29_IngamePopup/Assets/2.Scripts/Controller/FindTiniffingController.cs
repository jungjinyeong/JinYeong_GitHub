...
public class FindTiniffingController : MonoBehaviour
{
    ...
    private void TeeniepingMovement1_DOScale()
    {
        ...
        .SetAutoKill(false)
        .Prepend(teeniepingImageRectTr.DOScale(new Vector2(1.07f, 1.07f), 0.2f).SetLoops(6, LoopType.Yoyo)).SetDelay(2).OnComplete(() =>
        {
    ...
    private void TeeniepingMovement2_DOScale()
    {
        teeniepingMovementSequence = DOTween.Sequence()
        .SetAutoKill(false)
        .Prepend(teeniepingImageRectTr.DOScale(new Vector2(1.1f, 1.1f), 0.25f).SetLoops(8, LoopType.Yoyo)).SetDelay(4).OnComplete(() =>
        {
            teeniepingImageRectTr.DOScale(new Vector2(1f, 1f), 1f).SetEase(Ease.Linear).OnComplete(() =>
            {
                teeniepingMovementSequence.Restart();
    ...
    private void TeeniepingMovement3_DOScale()
    {
        teeniepingMovementSequence = DOTween.Sequence()
        .SetAutoKill(false)
        .Prepend(teeniepingImageRectTr.DOScale(new Vector2(1.15f, 1.15f), 0.3f).SetLoops(6, LoopType.Yoyo)).SetDelay(5).OnComplete(() =>
        {
            teeniepingImageRectTr.DOScale(new Vector2(1f, 1f), 1f).SetEase(Ease.Linear).OnComplete(() =>
            {
                teeniepingMovementSequence.Restart();
    ...
    private void TeeniepingMovement5_DOFade()
    {
        teeniepingMovementSequence = DOTween.Sequence()
        .SetAutoKill(false)
        .Prepend(teeniepingImage.DOFade((100f / 255f), 0.4f).SetEase(Ease.Linear).SetLoops(4, LoopType.Yoyo)).SetDelay(4).OnComplete(() =>
        {
