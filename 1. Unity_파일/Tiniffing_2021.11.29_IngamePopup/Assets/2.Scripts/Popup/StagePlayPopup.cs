...
public class StagePlayPopup : BasePopup
{
    ...
    private void StagePlayPopupDisplay()
    {
        ...
        .InsertCallback(1.5f, () => ItemGroupTweenDisplay())
        .InsertCallback(1.6f, () => ItemIconUpDownLoopTween())
        .Insert(1f, startGroupRectTr.DOAnchorPosX(516.2f, 0.7f)).SetEase(Ease.Linear)
        .InsertCallback(1.8f, () => StartButtonGroupRotateTween());
    }
    private void TitleTweenDisplay()
    {
        titleRectTr.DOScale(new Vector2(1.2f, 1.2f), 0.5f).OnComplete(() =>
        {
            titleRectTr.DOScale(new Vector2(1f, 1f), 0.5f);
        });
    }
    private void MissionTweenDisplay()
    {
        missionTweenDisplaySequence = DOTween.Sequence()
        .Prepend(teeniepingMindMissionTextImageRectTr.DOAnchorPosX(0, 0.4f).SetEase(Ease.Linear))
        .Insert(0.4f, teeniepingMindGroupTr.DOScale(new Vector2(1.2f, 1.2f), 0.2f))
        .Insert(0.5f, teeniepingMindGroupTr.DOScale(new Vector2(1f, 1f), 0.2f));
    }
    ...
    {
        startRotateSequence = DOTween.Sequence().SetDelay(3f)
        .Insert(0f, startButtonTweenGroupRectTr.DORotate(new Vector3(0, -360 * 2f, 0), 1.5f, RotateMode.WorldAxisAdd)).SetEase(Ease.OutQuad).OnComplete(() =>
        {
            startRotateSequence.Restart();
        });
    }
