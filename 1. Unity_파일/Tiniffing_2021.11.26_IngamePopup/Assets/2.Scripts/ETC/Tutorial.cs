...
public class Tutorial : MonoBehaviour
{
    ...
    IEnumerator Intro()
    {
        ...
        IntroSetting();
        lobbySpeechBubble0Text.DOText(DataService.Instance.GetText(950), 1.5f).SetEase(Ease.Linear).OnComplete(()=> { lobbySpeechBubble0_NextTextRect.gameObject.SetActive(true); });
        ...
        yield return new WaitUntil(() => isLobbyTouchScreenClick);
        teeniepingMindMask.SetActive(true); // 연타했을떄 예외처리
        ...
        lobbySpeechBubble0Text.text = "";
        lobbySpeechBubble0Text.DOKill();
        lobbySpeechBubble0Text.DOText(DataService.Instance.GetText(954), 1f).SetEase(Ease.Linear).OnComplete(() => { HeartspingMindRewardActive();});  // 티니핑 캐치를 터치해줘 츄 끝나고 => 티니핑캐치버튼 active 
    ...
    private void Repeat_LobbySpeechBubble0(int textId)
    {
        ...
        lobbySpeechBubble0_NextTextRect.gameObject.SetActive(false);
        isLobbyTouchScreenClick = false;
        lobbySpeechBubble0Text.DOText(DataService.Instance.GetText(textId), 1.5f).SetEase(Ease.Linear).OnComplete(() => { lobbySpeechBubble0_NextTextRect.gameObject.SetActive(true); }); 
    }
    ...
    IEnumerator AfterCompleteTeeniepingCatch()
    {
        ...
        lobbySpeechBubble1Text.DOText(DataService.Instance.GetText(955), 1.5f).SetEase(Ease.Linear).OnComplete(() => { lobbySpeechBubble1_NextTextRect.gameObject.SetActive(true); }); //우와 너무 잘하는데 ? ~~~~ 캐치한 티니핑을 알아보자
        yield return new WaitUntil(() => isLobbyTouchScreenClick);
        isLobbyTouchScreenClick = false;
    ...
    private void HandEffect2()//왼쪽 방향
    {
    ...
    private void HandEffect3()//오른쪽 방향
    {
    ...
    IEnumerator TeeniepingCollectionCor()
    {
        ...
        teeniepingCollectionSpeechBubble1Text.DOText(DataService.Instance.GetText(957), 1.5f).SetEase(Ease.Linear).OnComplete(() => 
        { 
