...
public class InGameUIInfo : MonoBehaviour
{
    ...
    private List<GameObject> spreadedCubeList= new List<GameObject>();

    private int cube_index; // spreadBellList 하나하나 순서대로 참조할 용도
    ...
    private Sequence boxEffectSequence;
    ...
    public void Init()
    {
        ...
        // 종 세팅
        CubeSpread();
        Debug.Log("list count : " + spreadedCubeList.Count +1 + " 개");
    }
    ...
    private void CubeEffectAndImageChange()
    {
        spreadedCubeList[cube_index].transform.GetChild(1).gameObject.SetActive(false);
        spreadedCubeList[cube_index].transform.GetChild(2).gameObject.SetActive(true); // effect on
        spreadedCubeList[cube_index].transform.GetChild(0).GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/InGameUI/cube_boxing");
        cube_index++; // index 증가
    }
    public void CubeImageChange()
    {
        RectTransform cubeRect= spreadedCubeList[cube_index].transform.GetChild(0).GetComponent<RectTransform>();
        RectTransform cubeLidRect = spreadedCubeList[cube_index].transform.GetChild(1).GetComponent<RectTransform>();

        boxEffectSequence = DOTween.Sequence()
        .Insert(0f, spreadedCubeList[cube_index].transform.GetChild(1).GetComponent<Image>().DOFade(1f, 0.3f)).SetEase(Ease.Linear)
        .Insert(0f, cubeLidRect.DOAnchorPosY(11f, 0.2f)).SetEase(Ease.OutCubic)//뚜껑 내려감
        .Insert(0.15f, cubeRect.DOAnchorPosY(-3.5f, 0.2f)).SetEase(Ease.Linear)//박스 내려감
        .Insert(0.05f, cubeLidRect.DOScaleX(1.09f, 0.2f)).SetEase(Ease.Linear)//뚜껑 늘어남
        .Insert(0.13f, cubeRect.GetComponent<RectTransform>().DOScaleX(1.09f, 0.05f)).SetEase(Ease.Linear)//박스 늘어남
        .Insert(0.35f, cubeLidRect.GetComponent<RectTransform>().DOAnchorPosY(14f, 0.05f)).SetEase(Ease.Linear)//뚜껑 올라감
        .Insert(0.35f, cubeRect.GetComponent<RectTransform>().DOAnchorPosY(0f, 0.05f)).SetEase(Ease.Linear)//박스 올라감
        .Insert(0.35f, cubeLidRect.GetComponent<RectTransform>().DOScaleX(1f, 0.05f)).SetEase(Ease.Linear)//뚜껑 사이즈 복귀
        .Insert(0.35f, cubeRect.GetComponent<RectTransform>().DOScaleX(1f, 0.05f)).SetEase(Ease.Linear)//박스 사이즈 복귀
        .OnComplete(() =>
        {
            CubeEffectAndImageChange();
        });
    }
    private void CubeSpread()
    {
       // 이미 값이 들어있었다면 List 값 초기화 (map 1 , map2 로 넘어갈때 생길 예외처리) 
        if (spreadedCubeList.Count !=0)
        {
            int cube_all_count = spreadedCubeList.Count;
           for (int i = 0; i < cube_all_count; i++)
           {
                Destroy(spreadedCubeList[0]); //게임 오브젝트 삭제
                spreadedCubeList.RemoveAt(0); // 0번째 인덱스 공간 삭제
        ...
        int cube_count =   GameManager.instance.GetSelectedTiniffingList().Count;
        int space = 0; //초기화
        int x;
        switch(cube_count)
        {
            case 3:
                for(int i =0; i<3; i++)
                {
                    GameObject cloneObj = Instantiate(Resources.Load<GameObject>("Prefabs/InGameUI/CubePrefab"), inGameUIGroupRectTr);
                     RectTransform cloneRectTr = cloneObj.GetComponent<RectTransform>();
                     x = -100;
                     cloneRectTr.anchoredPosition = new Vector2(x + space, -313);
                    space += 70;
                    spreadedCubeList.Add(cloneObj);
                }
                break;
            case 4:
                for (int i = 0; i < 4; i++)
