...
public class ScenarioController : MonoBehaviour
{
    ...
    IEnumerator ScriptCor()
    {
        for(int i = 0; i<resourceIdArr.Length; i++)
        {
            ...
            scriptText.DOText(DataService.Instance.GetText((int.Parse(textIdArr[i]))), 1.3f).SetEase(Ease.Linear).OnComplete(() => { nextButtonEffectObj.SetActive(true); });
            yield return new WaitUntil(()=>isNextButtonClick==true);
